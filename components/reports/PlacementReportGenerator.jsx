...
export default function PlacementReportEnhanced({ data, onRefresh }) {
  ...
  const applyFilters = () => {
    ...
    // Ensure salary is populated for placements by parsing jobDescription when needed
    const normalizedPlacements = (data.placements || []).map((placement) => {
      // If salary already present and parseable, keep it
      let salary = placement.salary;
      if (!salary) {
        // Try parsing structured jobDescription or free text
        try {
          if (placement.jobDescription) {
            // Try JSON first
            if (typeof placement.jobDescription === "string") {
              try {
                const jd = JSON.parse(placement.jobDescription);
                salary =
                ...
                // Free text fallback
                salary = parseSalaryFromText(placement.jobDescription) || null;
              }
            } else {
              salary = normalizeSalaryInput(placement.jobDescription) || null;
            }
          }
        } catch (e) {
          salary = null;
      ...
      return {
        ...placement,
        salary: salary || placement.salary || null,
      };
    });
    ...
    if (filters.company) {
      filtered = filtered.filter((placement) =>
        getCompanyDisplayName(placement.companyProfile, placement.companyName)
          .toLowerCase()
  ...
  const calculateStatistics = (placements) => {
    ...
    const companies = [
      ...new Set(
        placements.map((p) =>
          getCompanyDisplayName(p.companyProfile, p.companyName)
        )
      ),
    ];
    const placedWithSalary = placements.filter(
      (p) => p.salary && !isNaN(parseFloat(p.salary))
    );
  ...
  const generateAdvancedPDFReport = async () => {
    ...
    try {
      ...
      yPosition += 8;
      doc.text(
        `Report Period: ${filters.startDate || "All Time"} to ${
      ...
      yPosition += 8;
      doc.text(
        `Total Hired Students: ${currentData.placements.length}`,
        20,
        yPosition
      );
      yPosition += 20;
      ...
      yPosition += 8;
      doc.text(
        `Average Salary Package: Rs.${stats.averageSalary}`,
        20,
        yPosition
      );
      yPosition += 8;
      ...
      currentData.placements.slice(0, 35).forEach((placement) => {
        ...
        doc.text(
          (placement.studentName || "N/A").substring(0, 22),
          20,
          yPosition
        );
        doc.text(
        ...
        doc.text(
          (placement.jobTitle || "N/A").substring(0, 20),
          125,
          yPosition
        );
      ...
      if (currentData.placements.some((p) => p.salary)) {
        ...
        currentData.placements.slice(0, 20).forEach((placement) => {
          if (placement.salary && yPosition < 270) {
            doc.text(
              `${(placement.studentName || "N/A").substring(0, 25)}: Rs.${
                placement.salary
              }`,
              20,
              yPosition
      ...
      if (currentData.placements.length > 0) {
        ...
        currentData.placements.slice(0, 35).forEach((placement) => {
          ...
          doc.text(
            (placement.studentName || "N/A").substring(0, 25),
            20,
            yPosition
          );
          doc.text(
            (placement.studentEmail || "N/A").substring(0, 25),
            80,
            yPosition
          );
      ...
      if (currentData.placements.length > 40) {
        ...
        doc.setFontSize(10);
        doc.text(
          `... and ${currentData.placements.length - 40} more records`,
          20,
          yPosition
        );
        yPosition += 8;
        doc.text(
          "Export Excel for complete data with all fields",
          20,
          yPosition
        );
      }
    ...
    try {
      ...
      const summaryData = [
        ...
        [
          ...
          `Rs.${currentData.statistics.averageSalary}`,
        ],
      ...
      const detailData = [
        ...
        ...currentData.placements.map((p) => [
          ...
          p.facultyNo || "N/A",
          p.enrollmentNo || "N/A",
          getCompanyDisplayName(p.companyProfile, p.companyName) || "N/A",
          p.jobTitle || "N/A",
          p.status || "PENDING",
      ...
      XLSX.writeFile(
        wb,
        `placement-report-${new Date().toISOString().split("T")[0]}.xlsx`
      );
    } catch (error) {
      ...
      data.placements.map((p) => {
        try {
          const records = p.academicRecords
            ? typeof p.academicRecords === "string"
              ? JSON.parse(p.academicRecords)
              : p.academicRecords
            : {};
  ...
  return (
    <div className="space-y-6" style={{ color: "#111827" }}>
      {/*
        Advanced Filters & Report Options (commented out)
        The advanced filters UI has been intentionally disabled per request.
        If you need to re-enable it later, remove this JSX comment block.

      <div className="bg-gray-50 p-6 rounded-lg">
        <h3 className="text-lg font-semibold mb-4 text-gray-900">
          Advanced Filters & Report Options
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-800 mt-1">
              Only showing successfully hired/placed students
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div className="bg-green-50 p-4 rounded-lg border border-green-200">
          ...
          <p className="text-2xl font-bold text-green-900">
            {currentData.statistics.totalHired}
          </p>
        </div>
        ...
        <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
          ...
          <p className="text-2xl font-bold text-blue-900">
            {currentData.statistics.totalCompanies}
          </p>
        </div>
        ...
        <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
          ...
          <p className="text-2xl font-bold text-yellow-900">
            Rs.{currentData.statistics.maxSalary}
          </p>
        </div>
        ...
        <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-200">
          ...
          <p className="text-2xl font-bold text-indigo-900">
            Rs.{currentData.statistics.minSalary}
          </p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto max-h-96">
          ...
          {currentData.placements.length > 50 && (
            <div className="px-6 py-4 bg-gray-50 text-center text-sm text-gray-800">
              Showing first 50 records. Export full report to see all{" "}
              {currentData.placements.length} records.
            </div>
          )}
        </div>
      </div>
