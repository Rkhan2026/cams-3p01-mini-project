    ...
    try {
      ...
      if (jobsResult.success && applicationsResult.success) {
        ...
        const jobStats = {
          ...
          rejectedJobs: jobs.filter(job => job.approvalStatus === "REJECTED").length
        };

        // Calculate application stats
        const applicationStats = {
          totalApplications: applications.length,
          newApplications: applications.filter(app => app.applicationStatus === "APPLIED").length,
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-red-800">{stats.rejectedJobs}</div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white border rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-neutral-800">{stats.totalApplications}</div>
          <div className="text-xs text-neutral-600">Total Applications</div>
        </div>
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-center">
          <div className="text-2xl font-bold text-blue-800">{stats.newApplications}</div>
              ...
              {recentJobs.map((job) => (
                <div key={job.id} className="border rounded-md p-3 hover:bg-neutral-50">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-medium text-sm">Job #{job.id.slice(-8)}</h3>
            ...
            <div className="space-y-3">
              {recentApplications.map((application) => (
                <div key={application.id} className="border rounded-md p-3">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-medium text-sm">{application.student.name}</h3>
                    {getStatusBadge(application.applicationStatus, "application")}
                  </div>
      ...
      {stats.newApplications > 0 && (
        <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
          <h3 className="font-medium text-blue-800 mb-2">📬 New Applications</h3>
          <p className="text-sm text-blue-700">
            You have {stats.newApplications} new application{stats.newApplications > 1 ? "s" : ""} to review.
          </p>
        </div>
      )}
      ...
      {stats.pendingJobs > 0 && (
        <div className="mt-4 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <h3 className="font-medium text-yellow-800 mb-2">⏳ Pending Approval</h3>
          <p className="text-sm text-yellow-700">
            You have {stats.pendingJobs} job posting{stats.pendingJobs > 1 ? "s" : ""} awaiting TPO approval.
          </p>
        </div>
      )}
    </div>
