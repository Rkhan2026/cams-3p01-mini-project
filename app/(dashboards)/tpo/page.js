...
export default function TPODashboard() {
    ...
    try {
      ...
      if (
        ...
        const userStats = {
          ...
          totalUsers:
            pendingData.students.length + pendingData.recruiters.length,
        ...
        const applicationStats = {
          ...
          activeApplications: applications.filter((app) =>
            ["APPLIED", "SHORTLISTED", "INTERVIEW_SCHEDULED"].includes(
        ...
        setRecentJobs(
          jobs.filter((job) => job.approvalStatus === "PENDING").slice(0, 5)
        );
      }
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="p-6">
        <h1 className="text-black font-semibold mb-6">TPO Admin Dashboard</h1>
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-black font-semibold">TPO Admin Dashboard</h1>
      </div>

      {/* User Statistics */}
      <div className="mb-6">
        <h2 className="text-black font-medium mb-3">User Management</h2>
      ...
      <div className="mb-8">
        <h2 className="text-black font-medium mb-3">Placement Statistics</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border rounded-lg p-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-black font-semibold">Pending Registrations</h2>
            <Button
              onClick={() => router.push("/tpo/approvals")}
        ...
        <div className="bg-white border rounded-lg p-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-black font-semibold">Pending Job Approvals</h2>
            <Button
              onClick={() => router.push("/tpo/jobs")}
              className="bg-neutral-500 hover:bg-neutral-600 text-sm px-3 py-1"
            >
      ...
      {(stats.pendingStudents > 0 || stats.pendingRecruiters > 0) && (
        <div className="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          ...
          <p className="text-sm text-yellow-700">
            {stats.pendingStudents + stats.pendingRecruiters} user registration
            {stats.pendingStudents + stats.pendingRecruiters > 1 ? "s" : ""}
            {stats.pendingJobs > 0 &&
              ` and ${stats.pendingJobs} job posting${
      ...
      {stats.hiredStudents > 0 && (
        <div className="mt-4 bg-green-50 border border-green-200 rounded-lg p-4">
          ...
          <p className="text-sm text-green-700">
            {stats.hiredStudents} student
            {stats.hiredStudents > 1 ? "s have" : " has"} been successfully
            placed!
          </p>
        </div>
      )}
