      ...
      <div className="flex gap-2 mb-6 overflow-x-auto">
        {["ALL", "APPLIED", "SHORTLISTED", "INTERVIEW_SCHEDULED", "HIRED", "REJECTED"].map((status) => (
          ...
          >
            ...
            {status !== "ALL" && ` (${applications.filter(app => app.applicationStatus === status).length})`}
          </button>
        ))}
      </div>
            ...
            <div key={application.id} className="border rounded-lg p-6 hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-lg">{getStatusIcon(application.applicationStatus)}</span>
                    <h3 className="text-lg font-medium">{application.job.recruiter.name}</h3>
                    {getStatusBadge(application.applicationStatus)}
                  </div>
                  <p className="text-sm text-neutral-600">
                    Applied on: {formatDate(application.job.applicationDeadline)}
                  </p>
                </div>
              </div>
              ...
              {application.applicationStatus === "INTERVIEW_SCHEDULED" && (
                <div className="mt-4 bg-purple-50 border border-purple-200 rounded-md p-3">
                  ...
                  <p className="text-xs text-purple-600">Check your email for interview details and timing.</p>
                </div>
              )}

              {application.applicationStatus === "HIRED" && (
                <div className="mt-4 bg-green-50 border border-green-200 rounded-md p-3">
                  <p className="text-sm font-medium text-green-800">🎉 Congratulations! You've been hired!</p>
                  <p className="text-xs text-green-600">The recruiter will contact you with next steps.</p>
                </div>
              )}
              ...
              {application.applicationStatus === "REJECTED" && (
                <div className="mt-4 bg-red-50 border border-red-200 rounded-md p-3">
                  ...
                  <p className="text-xs text-red-600">Keep applying! There are more opportunities available.</p>
                </div>
              )}

              <div className="flex gap-2 mt-4">
                <Button
                  onClick={() => router.push(`/student/jobs/${application.job.id}`)}
                  className="bg-neutral-500 hover:bg-neutral-600 text-sm"
                >
                  View Job Details
                </Button>
              </div>
